...
<body>
    ...
    @endif
    <form method="POST" action="{{ route('redets.update', $returnDetail->id) }}">
        @csrf
        @method('PUT')
        <div>
            ...
            <select id="return_back_id" name="return_back_id" required>
                ...
                @foreach ($returnBacks as $returnBack)
                    <option value="{{ $returnBack->id }}" {{ $returnBack->id == $returnDetail->return_back_id ? 'selected' : '' }}>{{ $returnBack->return_code }}</option>
                @endforeach
            </select>
        </div>
        <div>
            ...
            <select id="borrow_id" name="borrow_id" required>
                ...
                @foreach ($borrowings as $borrowing)
                    <option value="{{ $borrowing->id }}" {{ $borrowing->id == $returnDetail->borrow_id ? 'selected' : '' }}>{{ $borrowing->borrow_code }}</option>
                @endforeach
            </select>
        </div>
        <div>
